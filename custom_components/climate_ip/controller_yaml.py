...
class StreamWrapper(object):
    def __init__(self, stream, token, ip_address, device_uid):
        self.stream = stream
        self.leftover = ""
        self.token = token
    ...
    def read(self, size):
        ...
        if count < size:
            ...
            if self.device_uid is not None:
                chunk = chunk.replace("__CLIMATE_DEVICE_ID__", self.device_uid)
            data += chunk
            count += len(chunk)
...
class YamlController(ClimateController):
    def __init__(self, config, logger):
        ...
        self._device_uid = config.get(CONFIG_DEVICE_INDEX)
        ...
        with open(file, "r") as stream:
            try:
                yaml_device = yaml.load(
                    StreamWrapper(stream, self._token, self._ip_address, self._device_uid),
